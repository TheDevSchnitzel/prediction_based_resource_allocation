    ...
    def one_hot_encode_history(self, df, dict_dir):
        ...
        for i in range(0, len(df)):
            ...
            if str(df.at[i, 'history']) != "nan" and str(df.at[i, 'res_history']) != "nan":
                ...
                # one hot encode X - Encode each historical step
                ...
                #zero-pad - Fill the history, such that each training data entry has the same history length 
                while len(onehot_encoded_X) != maxlen:
                    onehot_encoded_X.insert(0, [0]*(num_act+num_res))
                
                # Öhm, error? - Shuldn't even be possible by construction
        ...
        print("one_hot_encode_history shapes:")
        print("    X_train.shape: %s, y_a.shape: %s, y_t.shape: %s" % (str(X_train.shape), str(y_a.shape), str(y_t.shape)))
